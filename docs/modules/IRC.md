...
- NL User Chat Box Style（Neverlose用户聊天框样式）
 类型：枚举；默认："Text"
 说明：控制 Neverlose GUI 用户信息页下方 IRC 聊天框的显示样式。该聊天框不依赖 Chat Bar 模块开关，并会分页拉取服务端历史消息，每次 20 条、拉取冷却 5 秒；加载上一页后会保持在新旧消息交界处，历史内容不会作为新消息再次输出，消息按服务端消息 ID 去重，无法恢复的旧加密记录会被跳过。客户端启动读取本地缓存时会立即排除已撤回消息，并向服务端核对消息 ID，以服务端记录同步离线期间的撤回及清理存量重复缓存；缺少消息 ID 的旧版 IRC 缓存不会直接恢复。`Modern` 使用头像、等级、用户名/称号和左右气泡布局，并正确显示长消息及多行命令反馈；当天消息显示时分，昨天的消息显示“昨天 时分”，更早消息显示完整日期和时分；等级信息按用户缓存 30 分钟。自己的消息显示在右侧，并使用用户信息页头像。输入框支持鼠标定位、拖动选择及 Shift 延长选区；光标位于 `@` 后方时，会在输入框上方显示在线用户昵称自动补全列表。最后两条合规消息相同时，最后一条非本人消息右侧会出现 +1 按钮。Modern 消息支持悬停高亮、复制、翻译、本地删除和撤回；普通用户可撤回自己两分钟内的消息，管理组可撤回全部历史消息，同一消息的本地重复副本会一并替换，撤回后原位置会显示对应提示；若服务端确认消息已不存在，客户端会直接清理该消息及其本地缓存。
 可选：Text（文本）；Modern（现代）
## 历史更新
- v1.1.6：Neverlose 用户聊天框新增历史分页、鼠标选区、等级标识、+1 跟发和服务端消息撤回，并解除 Chat Bar 模块依赖。
