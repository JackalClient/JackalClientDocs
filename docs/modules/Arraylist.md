...
- Text Glow (PRO)（文本发光（专业版））
 类型：布尔；默认：true
 说明：这是开关型配置。默认值 true 代表作者推荐的初始行为；若要改动，建议一次只改一个开关便于观察影响。
- Text Glow Mode（文本发光模式）
 类型：枚举；默认："New"
 说明：Old 会逐条绘制传统文字发光；New 会通过独立的模糊资源高斯模糊完整文字层，在背景之后、清晰文字之前作为透明发光阴影绘制，不会改动其他 HUD 使用的模糊参数，模块较多时开销更低。New 会复用限制在屏幕范围内的贴图资源，关闭或回退时主动释放；无法创建模糊资源时会自动将本配置切回 Old。
 该配置始终显示；New 模式下同时关闭 Background 与 Text Glow 时，其他文本发光参数会自动隐藏。
 可选：Old（旧版）；New（新版）
- Text Glow Radius Rate（文本发光半径占比）
 类型：数值；默认：0.2f
 说明：控制 Old 的发光范围以及 New 的实际高斯模糊半径，数值越大，发光扩散越宽。
- Text Glow Alpha (0~1)（文本发光不透明度 (0~1)）
 类型：数值；默认：0.5f
 说明：控制 Old 与 New 的最终发光不透明度，数值越大，发光越明显；New 使用有上限的颜色增强并保留原始模糊透明度，避免重复提亮其他界面或形成不透明色块。饱和度补偿用于保留或增强发光颜色，默认值 1.0。
- Text Glow Saturation Compensation (0~2)（文本发光饱和度补偿 (0~2)）
 类型：数值；默认：1.0f
 说明：仅用于 New 模式。0 为灰度发光，1 保留原始颜色，2 提供更强的饱和度补偿，用于减少高亮合成造成的发白。
- Text Glow Enhancement Passes (2~40)（文本发光增强次数 (2~40)）
 类型：整数；默认：8
 说明：仅用于 New 模式。控制 shader 中有界颜色增强的等效次数，数值越大发光越强；有效范围为 2 至 40。
- Top（顶端）
 类型：数值；默认：60
