...
当 ProcessManager 能读取进程路径时，会额外检查可执行文件属性：如果 exe 处于隐藏状态，该进程行会显示红色边框，悬停提示中也会标记为高风险。PRO 版右键菜单支持 Show In FileManager，可在 FileManager 中新建标签页打开该进程所在目录。列表内也可以使用 W/S 上下移动当前选中项，并自动滚动到可见范围。
搜索框会在内容变化后立即刷新，并按 PID、进程名、描述、命令行等字段的相关性排列结果；CPU、GPU 与内存数值不参与关键词匹配，无匹配结果时会在列表中央显示提示。匹配文字会以粉色标出。输入框支持单击定位、拖动或 Shift 区间选择、双击选词和三击全选。进程右键菜单的 Target 下方提供“复制”子菜单，可复制进程名、PID、描述、进程路径、命令行或全部信息。鼠标经过列标题分界线时会显示左右调整光标。
...
类型：文本；默认：空；作用：不区分大小写地筛选进程，并优先排列 PID 与进程名匹配，其次排列描述和命令行匹配；输入 `pid:` 或 `pid=` 可精确筛选 PID；建议：先从进程名关键词开始，再叠加更细条件。
