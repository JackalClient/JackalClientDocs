...
- Command Highlighter（命令高亮器）
 类型：布尔；默认：true
 说明：这是开关型配置。默认值 true 代表作者推荐的初始行为；若要改动，建议一次只改一个开关便于观察影响。
- Preview Math Expression Highlight（预览数学表达式高亮）
 类型：布尔；默认：true
 说明：开启后，Shell 会提前异步判断当前输入是否像数学表达式；确认后会用独立颜色预览数字、字母、括号和运算符，并在输入变化时保持上一次判断结果直到下一次判断完成，避免可计算表达式被当作未知命令标红。
- Command Hints（命令提示）
 类型：布尔；默认：true
 说明：这是开关型配置。默认值 true 代表作者推荐的初始行为；若要改动，建议一次只改一个开关便于观察影响。
...
- bind（绑定）
 ...
 说明：该配置用于调整模块行为细节。建议先按默认值运行，确认需求后再逐步调整。
## 历史更新
- 55. 完善 Shell 输入高亮：开启 IRC 的 `Fast Send With # Prefix Command` 后支持 `#` 快捷发送格式预览、Quick Run 已保存程序提示，以及更灵敏的数学表达式预览高亮。
