...
任务运行过程由独立运行上下文管理，可区分暂停、等待模型、等待审批、执行工具、完成、失败和取消；暂停、恢复、取消和用户追加要求通过运行事件队列传递并与任务输入同步合并。
任务达到最长运行时间、用户停止或审批取消后，会在模型和工具调用边界检查取消状态，并在结束时清理 Agent 创建的交互式进程。
...
- Max Task Duration (s)（任务最长运行时间（秒））
 类型：数值；默认：1800
 说明：限制单次 Agent 任务的总运行时间。达到时限后任务会请求取消并进入终止流程，避免同步模型或工具调用无限占用任务生命周期。
