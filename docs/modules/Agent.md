...
AI代理
分类：Web
描述：使用人工智能代理和受控工具完成任务。

## 需求
- 安全级别：高风险模块
- 权限需求：根据工具操作可能需要管理员权限
- 驱动依赖：否
- 联网需求：是
- 开发状态：稳定/常规
- 版本属性：普通可用
...
## 介绍
Agent（AI代理）通过任务预设中允许的工具完成用户任务。
工具调用会在执行层校验任务范围和工具准备项，并按 effect、风险等级和 capability 统一授权。只读工具无需确认；命令、文件写入、交互进程、Jackal 指令、进程、窗口、音频输出及键盘操作等有副作用的工具会在执行前审批。
审批会展示规范化参数、目标资源和风险等级，文件写入还会显示内容差异。授权可仅生效一次，也可在本次任务内允许同一 capability；高风险操作不会因确认超时而自动放行。
任务运行过程由独立运行上下文管理，可区分暂停、等待模型、等待审批、执行工具、完成、失败和取消；用户在暂停期间追加的要求会与任务输入同步合并。
适合联网信息获取、账号联动、系统操作和在线交互场景。
...
- Command Confirmation（命令确认）
 类型：枚举；默认："MessageBox"
 说明：控制文件写入以外有副作用工具的统一审批方式。AI 不再负责审查自身操作风险。
 可选：Always Allow（始终允许）；MessageBox（消息框）；Console（控制台）
- File Confirmation（文件确认）
 类型：枚举；默认："MessageBox"
 说明：控制文件写入审批方式；审批内容包含路径、文件大小和内容差异预览。
 可选：Always Allow（始终允许）；MessageBox（消息框）；Console（控制台）
- Approval Grant Scope（审批授权范围）
 类型：枚举；默认："Once"
 说明：选择消息框批准仅作用于当前工具调用，或在本次 Agent 任务中允许同一 capability。新任务开始时会清空授权。
 可选：Once（仅一次）；Capability For Run（本次任务同类能力）
...
- Allow Tool Preparation Missing（允许工具缺少准备）
 类型：布尔；默认：false
 说明：控制非 general 任务是否允许在部分工具缺少准备项时启动。无论该项如何设置，缺少准备项的具体工具都不会执行；general 任务会隐藏不可用工具并继续运行。
## 历史更新
- v1.1.5：补充 Agent 运行上下文和明确状态记录，暂停与用户追加要求的处理更稳定。
- v1.1.5：引入统一工具策略与审批，按 effect、风险和 capability 区分工具，支持文件差异预览和本次任务同类能力授权。
- v1.1.5：加强任务工具范围和准备项校验，统一高风险工具确认，并修复文件拒绝、工具参数、次数限制和任务状态问题。
