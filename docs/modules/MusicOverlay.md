...
- Scale（整体缩放）
 类型：数值；默认：1.0f
 说明：统一缩放 MusicOverlay 的背景、封面、文字、波形和进度条；该尺寸不再跟随 ClickGUI Global Scale，建议小步调整，避免遮挡其他 HUD。
...
- Give Way to Mouse（给鼠标让道）
 类型：布尔；默认：true
 说明：这是开关型配置。默认值 true 代表作者推荐的初始行为；若要改动，建议一次只改一个开关便于观察影响。
- Give Way to Mouse Mode（给鼠标让道模式）
 类型：枚举；默认："Motion"
 说明：Motion 会沿用原有的位置避让；Fade 会在鼠标经过时平滑降低整个音乐面板的不透明度。
 可选：Motion（移动）；Fade（淡出）
- Give Way to Mouse Opacity (0~1)（给鼠标让道不透明度 (0~1)）
 类型：数值；默认：0.2
 说明：Fade 模式下鼠标经过时的目标不透明度。
- Background（背景）
 类型：布尔；默认：true
 说明：这是开关型配置。默认值 true 代表作者推荐的初始行为；若要改动，建议一次只改一个开关便于观察影响。
