...
- Weather（天气）
 ...
 可选：Off（关闭）；Rainy（雨天）；Snowy（雪天）；Thundery（雷雨天）；Sandstorm（沙暴）
- Weather Cycle（天气循环）
 类型：布尔；默认：true
...
- Sand Amount Scale（沙尘数量倍率）
 类型：数值；默认：1.2f
 说明：控制沙暴天气的沙尘密度，调高会增强效果但也会增加粒子数量。
- Sand Wind Speed（沙尘风速）
 类型：数值；默认：1.45f
 说明：控制沙尘横向移动速度，适合小步调整。
- Sand Fog Intensity (0~1)（沙暴雾化强度（0~1））
 类型：数值；默认：0.28f
 说明：控制沙暴覆盖屏幕的黄褐色雾化强度。
- Sand Darkness (0~1)（沙暴压暗强度（0~1））
 类型：数值；默认：0.08f
 说明：控制沙暴对画面的压暗程度。
- Sand Color（沙尘颜色）
 类型：枚举；默认："Gold"
 ...
 可选：见 [NAMED_COLOR_BASE_LIST](./NAMED_COLOR_BASE_LIST.md)
## 历史更新
- Ambience 新增沙暴天气，可显示横向沙尘、沙暴雾化与画面压暗效果。
- 50. 添加模块：Ambience。
