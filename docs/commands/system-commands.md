...
::: details 点击查看说明
枚举所有可用的人工智能代理任务预设。每一个任务严格规定可调用的工具范围，范围外的工具不会执行。
:::
...
::: details 点击查看说明
有一些工具需要准备。你可以用这条命令设置某项准备的值；准备项缺失时对应工具不会执行。
:::
...
::: details 点击查看说明
调用一个工具。参数必须用JSON表示。有副作用的工具会先经过统一授权，文件写入会显示内容差异；授权可仅生效一次，或在本次任务中允许同类能力。
:::
...
::: details 点击查看说明
人工智能代理，完成特定任务。注意人工智能可以在统一授权后调用命令和其他有副作用的工具操作你的计算机，高风险操作不会因确认超时而自动放行。task 为严格的工具范围，description 为任务具体需求。如果你想允许所有已准备好的工具，可以使用名为 general 的任务。
:::
