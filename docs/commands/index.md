...
[命令补全提示](/docs/public/command-completion.jpg)
3. 我加入了命令的自动高亮和提示信息，希望你能喜欢
   
## 如何输入命令

优先选择命令行，因为具有自动补全自动高亮等。

### 如何打开命令行
使用以下任一方法即可：
- ClickGUI 中 `Misc 其他` 一列中找到 `Shell 命令行`，左键启用，弹出的控制台窗口里即可输入命令。
- 右键托盘，在菜单中选择 `Open Shell 打开命令行`。
- 如果控制台已经显示，可以在标题栏右键，里面有 `Open Shell 打开命令行`，并且具有全局快捷键 `Ctrl+Alt+S`。
  
### 其他输入命令的入口（不是很推荐）
1. `Quick Command 快速命令` 模块可以允许你用快捷键弹出一个新输入框，里面输入命令后直接执行，无需打开控制台或命令行。
2. `Neverlose GUI` 的用户信息界面（左下角头像单击一次），选择 `Console 控制台` 标签页，里面也可以输入命令，但是暂时无法查看输出。
