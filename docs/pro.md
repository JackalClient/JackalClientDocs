...
- 按 Ctrl+Tab 可以快捷切换标签页
- 滚轮，↑↓，Home/End 都可以滚动或跳转。
- 加了很多丝滑的动画
- 每一个卡片，如果配置项很多，就具有内部滚动。
- 搜索是全局搜索，从所有模块中搜索。点击搜索框后右侧出现搜索模式，你可以搜索模块名称，也可以搜索配置项。按下 `Ctrl+M` 快速切换。
...
Q: 是否添加卡密验证系统
A: 二六年三月下旬已添加。详见《JackalClient 新版IRC和验证系统说明》。

Q: 有无其他内容
A: 已经陆陆续续在添加了，你可以在 `HISTORY.TXT` 中查找 `“【PRO】”`，你可以看到非常多的结果，都是专业版独有的内容。专业版独有的模块在免费版是灰色完全不可用的。
