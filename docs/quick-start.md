...
> 由于权限机制，如果客户端没有管理员权限，你是无法在管理员权限的窗口上开启 GUI的，例如任务管理器。你可以先把焦点转移给其他窗口然后再按按键，例如单击任务栏。
...
- `Ctrl + F`: 进入模块或模块配置项搜索界面
- `Ctrl + Tab`: 切换顶部标签页。注意，这可能和「应用」中的「文件管理器」冲突。
- `Win + ↑`: (v1.1.2) 若GUI打开，则最大化窗口（模拟出来的最大化）；若未打开且主窗口具有焦点，则为打开GUI。当然你也可以双击标题栏来最大化。
- `Win + ↓`: (v1.1.2) 最大化时，还原窗口（模拟出来的窗口化）；否则为关闭GUI。当然你也可以在最大化时拖动标题栏来窗口化。
- `Esc`: 也可以关闭 GUI
- 滚轮: 上下滚动当前页面。
- 上下箭头↑↓: 可以代替滚轮滚动。
...
所谓的`配置档案`就是客户端的参数配置文件。客户端会自动保存当前的配置。

- 点击 `Profile (档案)` 标签页可以打开配置档案管理器，可以加载、打开、删除、重命名配置档案。当前使用的配置档案会被标为绿色。
- v1.1.2起，你可以对配置档案右键弹出菜单，选择更多的选项进行操作。
- `default` 是默认配置档案，请勿删除。客户端启动时，会从 `DefaultSettings.dll` 获取默认配置并覆盖该文件。
